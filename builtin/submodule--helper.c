...
static void configure_added_submodule(struct add_data *info)
{
	...
	strvec_pushl(&add_submod.args, "--", info->sm_path, NULL);
	...
	free(key);
	if (info->branch) {
		key = xstrfmt("submodule.%s.branch", info->sm_path);
	...
	if (pathspec_key_exists && !submod_pathspec)
		warning(_("The submodule.active configuration exists, but "
			  ...
			  "submodule.%s.active to 'true'."), info->sm_name);
...
{
	const char *repo = NULL, *realrepo = NULL, *branch = NULL;
	const char *sm_path = NULL, *sm_name = NULL;
	int force = 0;
	...
	struct option options[] = {
		...
		OPT_STRING(0, "url", &repo,
			   N_("string"),
			   N_("url to clone submodule from")),
		OPT_STRING(0, "resolved-url", &realrepo,
			   N_("string"),
	...
	configure_added_submodule(&info);
